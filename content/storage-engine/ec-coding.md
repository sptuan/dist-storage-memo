...
- **Vandermonde 矩阵**：元素 $c_{i,j} = \alpha_i^{j}$，其中 $\alpha_i$ 是 $\text{GF}(2^8)$ 中互不相同的元素。这是最直观的构造——就是在不同点上对多项式求值。

- **Cauchy 矩阵**：元素 $c_{i,j} = \frac{1}{x_i + y_j}$，其中 $x_i, y_j$ 两两不同。Cauchy 矩阵的优势在于任意子矩阵天然可逆，且求逆运算可进一步优化为纯 XOR 操作[^cauchy_rs]。
...
$\text{LRC}(12, 2, 2)$ 共有 4 个校验块（2 局部 + 2 全局），但它**不能**容忍任意 4 个块同时丢失。考虑一个极端情况：$D_1, D_2, LP_1$ 同时丢失（局部组 1 内 3 个块），此时局部组 1 的局部校验已不可用，必须依赖全局校验。但全局校验只有 2 个，而需要恢复的块有 3 个（$D_1, D_2, LP_1$），超出了全局校验的能力。

准确地说，$\text{LRC}(12, 2, 2)$ 能够容忍：

- 任意 3 个块同时丢失（因为有 2 个全局 + 至少 1 个局部可用）
- 特定模式下的 4 个块丢失（如跨组各丢 1 个数据块 + 2 个全局校验块以外的组合）
- **不能**保证容忍任意 4 块丢失
